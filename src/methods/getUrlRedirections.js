...
/**
 * Gets the list of URL Redirections
 ...
 * @returns {Promise<AgilityFetch.Types.UrlRedirections>} - Array of URL Redirections returned by the response.
 * @example
 *
 ...
 * .then(function({ urlRedirections }) {
 *     console.log(urlRedirections);
 * })
...
function validateRequestParams(requestParams) {
  if (requestParams.lastAccessDate) {
    if (!requestParams.lastAccessDate.toISOString) {
      ...
      if (isNaN(dt)) {
        throw new TypeError(
          "You must include a valid Datetime for the lastAccessDate."
        );
      }
    }
  } else {
